...
#define TMO_TO_TICKS(ms) ((ms == OS_WAIT_FOREVER) ? portMAX_DELAY : (ms) / portTICK_PERIOD_MS)
...
{
    *value = xEventGroupWaitBits((EventGroupHandle_t)event,
                                 mask,
                                 pdFALSE,
                                 pdFALSE,
                                 TMO_TO_TICKS(time));
...
os_timer_t *os_timer_create(uint32_t us,
...
void os_timer_start(os_timer_t *timer)
{
    ...
    CC_ASSERT(xTimerChangePeriod(timer->handle, (timer->us / portTICK_PERIOD_MS) / 1000, portMAX_DELAY) == pdPASS);
}
...
void assert_report(bool exp, char const *file, char const *function, int line)
{
    if (!exp)
    {
        printf("assert! file:%s, fuction:%s, line:%05d\n", file, function, line);
        while (1)
        {
            vTaskDelete(NULL);
        }
